...
import io.apicurio.hub.api.fmpp.dataloaders.XmlDataLoader;
import io.apicurio.hub.api.fmpp.setting.Settings;
import io.apicurio.hub.api.fmpp.util.BugException;
import io.apicurio.hub.api.fmpp.util.MiscUtil;
import io.apicurio.hub.api.fmpp.util.*;
 ...
 * rather use a {@link Settings} object instead of directly using
 * this class.
 *
 * <p><b>{@link Engine fmpp.Engine} vs {@link Settings}</b>:
 ...
 * executing; attempting that will cause {@link IllegalStateException}.
...
public class Engine {
    ...
    private File srcRoot, outRoot, dataRoot, baseDir;
     ...
     *                    <li>{@code objectWrapper} to a {@link DefaultObjectWrapper}, if
    ...
    public void process(File[] sources, ZipOutputStream zos,String baseDir)
        ...
        try {
            ...
            try {
             ...
             //   out = new FmppFileOutputWriter(this);
                out = new FmppFileOutputWriter(this,zos);
                for (; currentTurn <= maxTurn; currentTurn++) {
                    for (int i = 0; i < srcs.length; i++) {
                        if (srcs[i] != null) {
                           ...
                           // Path relativePath = Paths.get(srcs[i].getPath().replace(File.separatorChar, '/'));
                         //   Path relativePath = Paths.get(baseDir+"/"+srcs[i].getPath());
                            if (isDirectory(srcs[i].getPath(),baseDir)) {
                                done = processDir(srcs[i], out, baseDir);
                            } else {
   ...
   private boolean isDirectory(String relativePath,String baseDir)
   {
...
/*        if (outRoot.exists() && !outRoot.isDirectory()) {
    ...
    private boolean processDir(File srcDir, File dstDir,String baseDir)
        ...
        if (!dontTraverseDirs) {
            ...
            for (int i = 0; i < dir.length; i++) {
                ...
                //sf.isDirectory()
                if (isDirectory(sf.getPath(),baseDir)) {
                    processDir(sf, df, baseDir);
                } else {
    ...
    private File[] listFiles(File srcDir,String baseDir) throws IOException {
        List<File> fileList = new ArrayList<>();
        Path directoryPath=Paths.get(baseDir+"/"+srcDir.getPath());
    ...
    public File[] listDirs(File srcDir, String chosenDirectory, Map<String, String> directoryPairs) throws IOException {
        File root = new File(baseDir.getPath()+"/"+srcDir);
        String excludedDirectory = directoryPairs.get(chosenDirectory);

        // Custom filter to exclude the specific directory
        DirectoryFileFilter customFilter = new DirectoryFileFilter() {
            @Override
            public boolean accept(File file) {
                return !file.getName().equals(excludedDirectory) && super.accept(file);
            }
        };

        Collection<File> files = FileUtils.listFilesAndDirs(root, TrueFileFilter.INSTANCE, customFilter);

       /* for (File file : files) {
            if (!file.isDirectory()) {
                // Process the file (e.g., read, modify or apply Freemarker templates)
    ...
    private boolean processFile(File sf, File df, boolean allowOutFAdj)
        ...
        try {
            ...
            case PMODE_COPY:
                ...
                FileUtil.copyFile(sf, df,out.getZipFileWriter());
                break;
    ...
    private void executeFile(File sf, File df)
       ...
       //  out = new FmppFileOutputWriter(this, df, outEnc);
       // out.setOutputEncoding(outEnc);
        out.setDst(df);
        out.initOutputBufferAndWriter();
        boolean done = false;
        try {
    ...
    public void setSourceRoot(File srcRoot) throws IOException {
        checkParameterLock();
        this.srcRoot = srcRoot != null ? srcRoot : null;
    }
    ...
    public void setDataRoot(File dataRoot)
            throws IOException {
        checkParameterLock();
        this.dataRoot = dataRoot != null ? dataRoot : null;
    }
     ...
     *     in error message of the {@link InstallationException}.
     *     Can be {@code null}.
     *
    ...
    private File adjustOutputFileName(File f) throws IOException {
        ...
        return new File(f.getParent(), fn);
    }
    ...
    private static void loadVersionInfo() {
        ...
        InputStream ins = Engine.class.getClassLoader()
                .getResourceAsStream("conf/version.properties");
        if (ins == null) {
