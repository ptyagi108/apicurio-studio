...
import io.apicurio.hub.api.fmpp.*;

import io.apicurio.hub.api.fmpp.localdatabuilders.BshLocalDataBuilder;
import io.apicurio.hub.api.fmpp.localdatabuilders.MapLocalDataBuilder;
import io.apicurio.hub.api.fmpp.localdatabuilders.TddHashLocalDataBuilder;
import io.apicurio.hub.api.fmpp.tdd.*;
import io.apicurio.hub.api.fmpp.util.BugException;
import io.apicurio.hub.api.fmpp.util.InstallationException;
import io.apicurio.hub.api.fmpp.util.MiscUtil;
import io.apicurio.hub.api.fmpp.util.StringUtil;
...
public class Settings {
     ...
     * {@link Fragment}. This is required because that hash parameter
     * may uses data loaders, which shouldn't be executed until almost all
     * settings of the {@link Engine} are set. This also means that if
     * you set the setting value with Java, the last parameter to the
     * <tt>case</tt> {@link FunctionCall} must be a
     * {@link Fragment} instead of <code>Map</code>, if you want to use
     * data loaders in it. 
     */
     ...
     *      <code>Map</code>, <code>String</code>, {@link Fragment}.
     * <p>String input: TDD expression, starting in hash mode.
     ...
     */
    public Settings(File baseDir) throws SettingException, IOException {
        if (baseDir == null) {
     ...
     *     coming from the file.
     */
    public Settings(File baseDir,CodegenProject project) throws SettingException, IOException {
        if (baseDir == null) {
            throw new IllegalArgumentException(
                    "Parameter \"baseDir\" can't be null.");
        }
     ...
     * For each call of this method, a new {@link Engine} object will be
     ...
     * that for each call of this method, a new {@link Engine}
    ...
    public void execute(ZipOutputStream zos) throws SettingException, ProcessingException, IOException {
        ...
        */
        ...
        if (!eng.getExpertMode()) {
            if (outputFile == null) {
                if (eng.getSourceRoot()
                        .equals(eng.getOutputRoot())) {
                    throw new SettingException(
    ...
    /**
     * Executes the processing session(s) on the {@link Engine} level,
    ...
    private void load_inner(
        ...
        try {
            ...
            if (tddMode) {
             ...
             InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream(s);
                try {
    ...
    private File load_getMetaSetting(Map m, String name, boolean tddMode)
        ...
        if (o != null) {
            ...
            if (f.isAbsolute()) {
                ...
                return new FileWithSettingValue(
                        new File(baseDir, f.getPath()).getPath(),
                        (String) o);
            }
        } else {
            return null;
    ...
    private void loadXmlCatalogs(
        ...
        Iterator it = ls.iterator();
        while (it.hasNext()) {
            catalogs.append(((File) it.next()).getPath());
            if (it.hasNext()) {
                catalogs.append(';');
            }
        }
