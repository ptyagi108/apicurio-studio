...
package io.apicurio.hub.api.fmpp.tools;

import freemarker.log.Logger;
import freemarker.template.Configuration;
import freemarker.template.Version;
import io.apicurio.hub.api.fmpp.Engine;
import io.apicurio.hub.api.fmpp.ProcessingException;
import io.apicurio.hub.api.fmpp.progresslisteners.ConsoleProgressListener;
import io.apicurio.hub.api.fmpp.progresslisteners.LoggerProgressListener;
import io.apicurio.hub.api.fmpp.progresslisteners.StatisticsProgressListener;
import io.apicurio.hub.api.fmpp.progresslisteners.TerseConsoleProgressListener;
import io.apicurio.hub.api.fmpp.setting.FileWithSettingValue;
...
public class CommandLine {
    ...
    private void printHelp(ArgsParser ap) {
        p("Typical usages:");
        p("appgen -C configfile", 3);
        p("appgen -S sourcedir -O outputdir", 3);
        p("appgen sourcefile -o outputfile", 3);
      //  p("For more examples: http://fmpp.sourceforge.net/commandline.html");
        if (ap == null) {
            p("To see all options: appgen -h");
        } else {
            p();
            p("Options:");
