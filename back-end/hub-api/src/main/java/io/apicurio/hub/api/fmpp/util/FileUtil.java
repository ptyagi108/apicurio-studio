...
public class FileUtil {
     ...
     */
    public static void copyFile_bak(File src, File dst, boolean copyLMD)
            throws IOException {
     ...
     *     file will be copied too.
     */
    public static void copyFile(File src, File dst, ZipOutputStream zipOut, boolean copyLMD)
            throws IOException {
        byte[] buffer = new byte[1024 * 64];
        InputStream in = new FileInputStream(src);
        try {
            ...
            zipOut.putNextEntry(new ZipEntry(dst.getPath()));
          //  OutputStream out = new FileOutputStream(dst);
            try {
                int ln;
                while ((ln = in.read(buffer)) != -1) {
                    zipOut.write(buffer, 0, ln);
                }
            } finally {
               // zipOut.close();
            }
    ...
    /**
     * Same as {@link #copyFile(File, File, ZipOutputStream, boolean) copyFile(src, dst, true))}.
     */
    public static void copyFile(File src, File dst, ZipOutputStream zipOut  ) throws IOException {
        copyFile(src, dst, zipOut,true);
    }
